...
**Purpose:** identify important EMOPET files that were sent, received or prepared, even when the binary itself is stored outside GitHub.
...
| 2026-09 | SENT | Prof. James Serpell / UPenn | `C-BARQ — contexts relevant to EMOPET ELI behavioral state interpretation.pdf` | `SENT` | 116909 B | hash not yet imported | Gmail thread `19cf199a8234f2fc` |
| 2026-09 | SENT | Prof. James Serpell / UPenn | `Controlled Research Brief — ELI Co-Pilot Training Proposal.pdf` | `SENT` | 181260 B | hash not yet imported | Gmail thread `19cf199a8234f2fc` |
...
1. Never change `PREPARED_NOT_SENT` to `SENT` without transmission evidence.
2. Never infer a supplier/customer/scientific acknowledgement from an outbound message.
3. Signed legal originals containing private addresses/signatures should normally remain in the controlled legal archive; GitHub stores hash + status + archive reference.
4. Add SHA-256 when exact bytes become available.
5. Superseded documents remain in the ledger with their historical status; do not silently delete evidence history.
