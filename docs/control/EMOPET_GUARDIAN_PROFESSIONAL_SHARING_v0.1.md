# EMOPET — Guardian-Controlled Professional Sharing v0.1

**Status:** CONTROLLED DESIGN AUTHORITY / PRE-PRODUCTION / NOT SECURITY VALIDATION  
**Date:** 2026-09-06  
**Parent:** #64 Guardian Authority  
**Related:** #223 Experience Hardening, Veterinary Summary / prescriber strategy  
**Gate:** `G-GUARDIAN-PROFESSIONAL-SHARE-01 = OPEN`

## 0. Purpose

This document replaces the idea that a single `vet_export_opt_in = true` can authorize veterinary/professional access.
...
> **A Guardian shares a defined view with a defined recipient for a defined reason and period. EMOPET never turns a general preference into permanent professional access.**
...
### Trusted/household Guardian

No automatic professional-sharing authority.

A future delegation may explicitly grant `MANAGE_PROFESSIONAL_SHARING`, but household membership alone is insufficient.
...
- unrelated dogs;
- unrelated Guardian account data.
...
Extending the data period MUST require a new/updated Guardian decision.
...
The Guardian-facing audit should answer:
...
- revocation must not delete the underlying Guardian-owned dog data.

## 8. Breiz language authority

Breiz may:
- explain what the Guardian is about to share;
- explain scope/expiry in plain language;
- remind the Guardian that sharing is optional/revocable.
...
- a Guardian cannot share raw/private scopes not exposed by policy;
...
The Guardian should see a review screen before creation:
