...
Reconstructed on current `main`: `2026-09-22`
Snapshot boundary: `main@3fa5c5298247fc88412ce2c8294fdb4f36024f56`
Original snapshot boundary: `main@c099581ff8aed1e619f72ab38898fc05833b7c66`
...
This register was drafted against `main@c099581` and stranded in an unmerged PR while `main` moved far ahead. It is re-anchored here rather than replayed: **every `VERIFIED_POINTER` below was re-checked against the current tree**, because a pointer carried forward without re-verification is no longer verified.

Result of that re-check:

| Claim | Current state |
|---|---|
| Twelve non-Markdown assets under `apps/web/public/assets/brand/` | **unchanged** — same twelve filenames |
| All twelve brand Git blob hashes | **12 of 12 unchanged** |
| `social-preview.png` byte-identical to `branding-fr.png` | **still true** — both `c2be7a68…` |
| `docs/architecture/data_flow_diagram.png` is a lone-newline blob | **still true** — `8b137891…`, one byte |
| `AGENTS.md` names Cédric Mian and Mohamed in the stated roles | **still true** |

No disposition changed. Every gate that was `OPEN` remains `OPEN`; the reconstruction adds evidence currency, not closure.
...
|---|---|---|---|---|
| IP-SRC-001 | Repository snapshot | `main@3fa5c529` (re-anchored 2026-09-22) | Snapshot identity for this register | `VERIFIED_POINTER` |
...
| IP-SRC-003 | Named project roles | `AGENTS.md` | Repository instructions name Cédric Mian as Founder/CEO and Mohamed as CTO — re-verified 2026-09-22 | `VERIFIED_POINTER` |
| IP-SRC-004 | Brand origin notes | `apps/web/public/assets/brand/README.md` | Distinguishes supplied presentation assets from placeholder SVG paths; does not name creators or rights instruments | `VERIFIED_POINTER` |
| IP-SRC-005 | Brand directory inventory | current tree | Twelve non-Markdown asset files are present under the brand directory — re-verified 2026-09-22 | `VERIFIED_POINTER` |
| IP-SRC-006 | Architecture PNG path | `docs/architecture/data_flow_diagram.png`, blob `8b137891791fe96927ad78e64b0aad7bded08bdc` | The tracked blob contains one newline byte, not an image payload — re-verified 2026-09-22 | `VERIFIED_POINTER` |
| IP-SRC-007 | Report attribution | PR #112 commit `92e440bc4e6df44644d0841ccf3f8b2cdda029a6` | The report commit contains a `Co-Authored-By: Claude Opus 5` trailer and a Claude session pointer | `VERIFIED_POINTER` |
...
| IP-CODE-008 | Seeds/reference content authored inside code files | Source files are versioned, including local-directory seed material | `OPEN` | `OPEN` | Third-party data rights excluded from this gate | `OPEN` | Record authorship here; route source-data rights to #116 |
...
| IP-PER-005 | PR #112 AI assistance | Commit trailer and session pointer are present after the original snapshot | Report drafting only, as represented by the PR metadata | PR #112 | `VERIFIED_POINTER` for attribution only | Preserve snapshot boundary; do not generalize this attribution to earlier material |
...
Every blob hash below was re-verified against `main@3fa5c529` on 2026-09-22: **twelve of twelve unchanged** since `c099581`.

| Asset ID | Repository path | Git blob | Repository-observable fact | Fact state | Creator/right state | Required evidence |
|---|---|---|---|---|---|---|
...
| IP-ASSET-012 | `apps/web/public/assets/brand/social-preview.png` | `c2be7a68307b62c5716bf95910962c772a8a9a0e` | Byte-identical Git blob to `branding-fr.png` — re-verified 2026-09-22 | `VERIFIED_POINTER` | `OPEN` | Confirm intentional reuse and source/rights coverage for social media |
| IP-ASSET-013 | `docs/architecture/data_flow_diagram.png` | `8b137891791fe96927ad78e64b0aad7bded08bdc` | Blob contains only a newline; no image payload exists | `VERIFIED_POINTER` | `NOT_APPLICABLE` to current empty payload; future diagram `OPEN` | Remove/replace only in a separate authorized change; record provenance of any future diagram |
...
No decision is made by this register.
...
`IP PROVENANCE REGISTER = RECONSTRUCTED ON CURRENT MAIN, POINTERS RE-VERIFIED`
