...
**Parent:** #64 (historical workstream number retained)  
**Legacy stable gate identifier:** `G-GUARDIAN-AUTHORITY-01 = OPEN`

> This is the canonical terminology successor for the controlled authorization model. The legacy gate identifier remains stable for evidence continuity. This revision changes role names, not authority scope.

## 0. Core principle

The Owner–dog relationship is the primary authorization boundary for personal EMOPET data and device authority.
...
Capabilities must never be inferred from relationship labels such as partner, family, sitter or friend. A trusted caregiver does not become an Owner by receiving a delegation.

### Professional Recipient
Access only through professional-share grant authority. Not an Owner.

### Community / World Principal
Never inherits Care/Owner authority from social graph, Circle membership, proximity, co-presence or World interaction.
...
- create long-lived Trusted Caregiver delegation;
...
|---|---:|---:|
| Primary Owner | Yes, backend policy | Yes, scoped grant |
| Explicit Trusted Caregiver capability | Only granted capabilities | Only if explicitly granted |
...
Before the legacy stable gate `G-GUARDIAN-AUTHORITY-01` can close:
