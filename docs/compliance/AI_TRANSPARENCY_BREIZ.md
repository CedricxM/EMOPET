...
Server responses include transparency metadata describing whether the answer used a model or retrieval fallback, the evidence level and (on the model path) provider/model id. Product UI may expose provider/model details in an information panel rather than cluttering every message, but the AI identity itself must remain visible.
