import {
  ...
  type BreachAffectedSurface,
  type BreachRecipientSurfaceDescriptor,
} from './breach-recipient-surfaces.generated.js';

export { BREACH_RECIPIENT_SURFACES } from './breach-recipient-surfaces.generated.js';
...
export type BreachRecipientResolution =
  | { status: 'RESOLVED'; personRefs: readonly string[] }
  | { status: 'NO_PERSON_REPRESENTED' }
  | { status: 'UNRESOLVED'; gap: BreachRecipientGapReason };
...
export type BreachRecipientEnumeration =
  | { status: 'INVALID_INPUT'; recipients: readonly []; gaps: readonly [] }
  | {
...
function parseAffectedObject(value: unknown): BreachAffectedObject | null {
  ...
  if (typeof value.surface !== 'string' || !SURFACE_BY_ID.has(value.surface)) return null;
  if (typeof value.ref !== 'string' || !SAFE_REF_RE.test(value.ref)) return null;
  return { surface: value.surface as BreachAffectedSurface, ref: value.ref };
}

function parseUnsupportedObject(value: unknown): { surface: string; ref: string } | null {
  if (!isRecord(value) || !hasOnlyKeys(value, OBJECT_KEYS)) return null;
  if (typeof value.surface !== 'string' || value.surface.length === 0 || SURFACE_BY_ID.has(value.surface)) return null;
  if (typeof value.ref !== 'string' || !SAFE_REF_RE.test(value.ref)) return null;
...
function isSafePersonRef(value: unknown): value is string {
  ...
  if (value.startsWith('user:')) return UUID_RE.test(value.slice('user:'.length));
...
function parseResolution(value: unknown): BreachRecipientResolution | null {
  ...
  if (value.status === 'NO_PERSON_REPRESENTED') {
    return hasOnlyKeys(value, NO_PERSON_KEYS) ? { status: 'NO_PERSON_REPRESENTED' } : null;
  }
  ...
  if (value.status === 'UNRESOLVED') {
    ...
    return { status: 'UNRESOLVED', gap: value.gap as BreachRecipientGapReason };
  }
...
function keyFor(object: BreachAffectedObject): string {
  return object.surface + ':' + object.ref;
}

function requiredGapFor(surface: BreachAffectedSurface): BreachRecipientGapReason | null {
  const descriptor = SURFACE_BY_ID.get(surface);
  return (descriptor?.requiredGap ?? null) as BreachRecipientGapReason | null;
}
...
export async function enumerateBreachRecipients(
  ...
  affectedObjects.sort((left, right) => keyFor(left).localeCompare(keyFor(right)));
  ...
  for (const object of affectedObjects) {
    ...
    try {
      ...
      gaps.push({ objectKey: keyFor(object), reason: 'resolver_failure' });
      continue;
    }
    ...
    if (resolution.status === 'UNRESOLVED') {
      gaps.push({ objectKey: keyFor(object), reason: resolution.gap });
      continue;
    }
    ...
    const authorityGap = requiredGapFor(object.surface);
    if (authorityGap) gaps.push({ objectKey: keyFor(object), reason: authorityGap });
  }
