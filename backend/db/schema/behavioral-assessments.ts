 ...
 * - Household/cohabitation context is snapshotted on the assessment so later
 *   household changes cannot rewrite the context in which answers were given.
 */
...
export const behavioralAssessments = pgTable('behavioral_assessments', {
  ...
  // Historical administration-context snapshot. Nullable for pre-existing
  // assessments; when present it must be versioned and timestamped.
  householdDogCount: integer('household_dog_count'),
  householdContextVersion: varchar('household_context_version', { length: 100 }),
  ...
  check(
    'chk_behavioral_assessment_context_pair',
    sql`(${table.householdContextVersion} IS NULL) = (${table.householdContextCapturedAt} IS NULL)`,
  ),
  check(
    'chk_behavioral_assessment_context_authority',
    sql`(${table.householdDogCount} IS NULL AND ${table.cohabitationContext} IS NULL)
      OR (${table.householdContextVersion} IS NOT NULL AND ${table.householdContextCapturedAt} IS NOT NULL)`,
  ),
  check(
    'chk_behavioral_assessment_cohabitation_context_shape',
    sql`${table.cohabitationContext} IS NULL OR jsonb_typeof(${table.cohabitationContext}) = 'object'`,
  ),
]);
