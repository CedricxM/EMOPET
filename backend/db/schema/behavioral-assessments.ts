 ...
 *   explicitly rather than silently imputed or collapsed to zero.
 * - Household applicability context is snapshotted on the assessment; later
 *   household changes must not rewrite the historical administration context.
 */
...
export const behavioralAssessments = pgTable('behavioral_assessments', {
  ...
  // Snapshot of applicability context at administration time. These fields are
  // historical evidence and must not be backfilled from a dog's current household.
  ...
  contextVersion: varchar('context_version', { length: 50 }),
  ...
  check(
    ...
    sql`(
      (${table.householdDogCount} IS NULL AND ${table.multiDogHousehold} IS NULL)
      OR
      (
        ${table.householdDogCount} >= 1
        AND ${table.multiDogHousehold} = (${table.householdDogCount} > 1)
        AND ${table.contextVersion} IS NOT NULL
