...
export const professionalShareGrants = pgTable('professional_share_grants', {
  ...
  index('idx_prof_share_grant_owner_dog').on(table.ownerUserId, table.dogId),
  ...
  check(
    ...
    sql`CASE WHEN jsonb_typeof(${table.scopes}) = 'array' THEN
      jsonb_array_length(${table.scopes}) BETWEEN 1 AND 5
      AND ${table.scopes} <@ '["VETERINARY_SUMMARY","OWNER_SELECTED_NOTES","QUALIFIED_LONGITUDINAL_OBSERVATIONS","DATA_COVERAGE_AND_CONFIDENCE","DECLARED_CONTEXT"]'::jsonb
      ELSE false END`,
  ),
  ...
  check(
    ...
    sql`(${table.status} <> 'ACTIVE' OR (${table.recipientPrincipalId} IS NOT NULL AND ${table.activatedAt} IS NOT NULL))
