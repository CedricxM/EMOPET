...
test('alert builder rejects detection-type and actor-key mismatches', () => {
  assert.equal(buildSecurityAlertCandidate({
    type: 'machine_privileged_authority_attempt',
    actorKey: 'privileged_human:11111111-1111-4111-8111-111111111111',
    occurredAt: '2026-09-04T09:04:00Z',
  }, policy(), '2026-09-04T09:05:00Z').status, 'INVALID_DETECTION');
...
test('alert builder rejects detected-at timestamps before the source detection chronology', () => {
  assert.deepEqual(buildSecurityAlertCandidate(detection(), policy(), '2026-09-04T09:03:59Z'), {
    status: 'INVALID_TIMESTAMP',
    alert: null,
  });

  const alert = builtAlert();
  assert.equal(evaluateSecurityAlertResponse({
    ...alert,
    sourceOccurredAt: '2026-09-04T09:06:00Z',
  }, '2026-09-04T09:07:00Z').status, 'INVALID_ALERT');
});
