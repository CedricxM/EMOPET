...
test('Owner data export preserves authorization and disclosure through JSON/CSV delivery', {
  ...
  await sql`INSERT INTO devices (id, dog_id, type, mac_address, firmware_version)
    VALUES
      (${summaryDeviceId}, ${dogId}, 'TAG', '02:00:00:00:04:01', 'summary-fixture-fw'),
      (${otherSummaryDeviceId}, ${otherDogId}, 'MAT', '02:00:00:00:04:02', 'other-summary-fw')`;
  ...
  for (const timestamp of ['2026-08-31T23:59:59Z', from, '2026-09-02T00:00:00Z', to, '2026-09-03T00:00:01Z']) {
    ...
    await sql`INSERT INTO sensor_summaries
      (id, dog_id, ingestion_id, device_id, timestamp, source, activity_minutes, temperature_c)
      VALUES (${id}, ${dogId}, ${ingestionId}, ${summaryDeviceId}, ${timestamp}, 'TAG', 12.5, -1.5)`;
  ...
  await sql`INSERT INTO sensor_summaries
    (id, dog_id, ingestion_id, device_id, timestamp, source, activity_minutes)
    VALUES (${otherSummaryId}, ${otherDogId}, ${otherSummaryIngestionId}, ${otherSummaryDeviceId}, ${from}, 'MAT', 999)`;
  ...
  await t.test('actual JSON and CSV enforce the same window, identity and publication projection', async () => {
    ...
    for (const row of json.preprocessed) {
      assert.equal(Object.hasOwn(row, 'ingestionId'), false, 'retry identity must stay internal');
    }
    ...
    for (const output of [JSON.stringify(json), csvText]) {
      for (const forbidden of [...outsideIds, ...summaryIngestionIds, otherSummaryIngestionId, excludedEliId, otherSummaryId, otherDogId, 'PRIVATE-MODEL-STATE', 'PRIVATE-BASELINE-METRICS', 'PRIVATE-PASSWORD-FIXTURE', ...deviceFixtures.map((row) => row.mac)]) {
  ...
  await t.test('a transfer already in flight cannot release an export to the former owner', async () => {
    ...
    try {
      ...
      for (const response of await Promise.all(pending)) {
        assert.ok([404, 503].includes(response.status), 'former Owner must fail closed after winning transfer');
        const body = await response.json();
        if (response.status === 503) {
          assert.equal(body.code, 'DATA_EXPORT_UNAVAILABLE');
