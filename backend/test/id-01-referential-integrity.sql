...
BEGIN
  FOREACH tbl IN ARRAY ARRAY[
    ...
    IF NOT has_fk THEN
      RAISE EXCEPTION 'ID-01 missing NO ACTION dog FK on %.dog_id', tbl;
    END IF;
  END LOOP;
  ...
  IF NOT has_fk THEN
    RAISE EXCEPTION 'ID-01 missing NO ACTION user FK on user_config.user_id';
  END IF;
END $$;
...
BEGIN
  BEGIN
    ...
    RAISE EXCEPTION 'ID-01 expected referenced ELI dog delete to fail';
  EXCEPTION
    WHEN foreign_key_violation THEN
      NULL;
  END;
