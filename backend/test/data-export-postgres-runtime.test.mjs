...
test('Owner data export preserves authorization and disclosure through JSON/CSV delivery', {
  ...
  const sensorDeviceId = randomUUID();
  const otherSensorDeviceId = randomUUID();
  await sql`INSERT INTO devices (id, dog_id, type, mac_address, firmware_version)
    VALUES
      (${sensorDeviceId}, ${dogId}, 'TAG', '02:00:00:04:00:01', '1.2.3'),
      (${otherSensorDeviceId}, ${otherDogId}, 'MAT', '02:00:00:04:00:02', '2.0.0')`;
  ...
  for (const timestamp of ['2026-08-31T23:59:59Z', from, '2026-09-02T00:00:00Z', to, '2026-09-03T00:00:01Z']) {
    ...
    await sql`INSERT INTO sensor_summaries (
      ...
      ${id}, ${dogId}, ${ingestionId}, ${sensorDeviceId}, ${timestamp}, 'TAG',
      '1.2.3', 12.5, -1.5
    )`;
  ...
  await sql`INSERT INTO sensor_summaries (
    ...
    ${otherSummaryId}, ${otherDogId}, ${otherSummaryIngestionId},
    ${otherSensorDeviceId}, ${from}, 'MAT', '2.0.0', 999
  )`;
  ...
  await t.test('actual JSON and CSV enforce the same window, identity and publication projection', async () => {
    ...
    for (const row of json.preprocessed) {
      assert.equal(Object.hasOwn(row, 'ingestionId'), false);
      assert.equal(row.deviceId, sensorDeviceId);
      assert.equal(row.firmwareVersionAtIngest, '1.2.3');
      assert.equal(row.provenance.deviceBinding, 'SERVER_VERIFIED_REGISTRY_BINDING');
      assert.equal(row.provenance.firmwareSnapshotSource, 'SERVER_DEVICE_REGISTRY');
    }
    ...
    for (const output of [JSON.stringify(json), csvText]) {
      for (const forbidden of [
        ...outsideIds,
        ...summaryIngestionIds,
        otherSummaryIngestionId,
        excludedEliId,
        otherSummaryId,
        otherDogId,
        'PRIVATE-MODEL-STATE',
        'PRIVATE-BASELINE-METRICS',
        'PRIVATE-PASSWORD-FIXTURE',
        ...deviceFixtures.map((row) => row.mac),
      ]) {
