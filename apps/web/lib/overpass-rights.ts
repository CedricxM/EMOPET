 ...
 * repository authority stays HOLD until the exact live-query/provider policy and
 * rendered attribution evidence have been reviewed for the current flow.
 *
 * The current implementation permits only an ephemeral in-memory cache. Export
 * and persistent/derived-database use are explicitly outside this authority.
 */
export const OVERPASS_PRODUCTION_AUTHORITY: OverpassReleaseAuthority = {
  ...
  reason:
    'Public Overpass provider/use policy and controlled rendered-attribution evidence remain open under #116.',
};
